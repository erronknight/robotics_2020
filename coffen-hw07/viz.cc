...
void viz_draw_path(deque<pathNode> path) {
	int ww = 700;
	int hh = 700;
	int dd = min(ww, hh) / 2;
	...
	if (!draw_over.empty()) {
		gfx_color(0, 100, 100);
		for (std::deque<pathNode>::iterator itd = draw_over.begin(); itd != draw_over.end(); ++itd) {
			yy_fin = dd + (-1 * itd->x);
			xx_fin = dd + (-1 * itd->y);
			gfx_point(xx_fin, yy_fin);
		}
	...
	gfx_color(200, 200, 0);
	for (std::deque<pathNode>::iterator it = path.begin(); it != path.end(); ++it) {
		yy_fin = dd + (-1 * it->x);
		xx_fin = dd + (-1 * it->y);
		gfx_point(xx_fin, yy_fin);
	}
